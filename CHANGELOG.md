...
- Added a hint for an unknown command that a package provides. Without `phalcon/migrations`, `crest migration:run` now prints `unknown command 'migration:run'; provided by phalcon/migrations`. When a command with that prefix is registered, the package is installed, so a wrong name gets the plain message. `Crest\Console\Registry::withProviders()` takes the map of name prefix to package name; crest gives it in `Crest\Commands`. [#19](https://github.com/phalcon/crest/issues/19)
